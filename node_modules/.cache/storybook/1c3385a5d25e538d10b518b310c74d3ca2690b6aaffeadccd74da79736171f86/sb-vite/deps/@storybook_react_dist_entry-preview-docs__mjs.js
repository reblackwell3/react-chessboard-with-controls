...
} from "./chunk-E2NR3O7G.js";
import {
  require_isString
} from "./chunk-I27OTZBW.js";
import {
  require_escodegen
} from "./chunk-RNIT6CZY.js";
import "./chunk-J4Q7VKTU.js";
import "./chunk-LUK4PL4A.js";
import {
  require_html_tags
} from "./chunk-L7HEUSNN.js";
import {
  require_dist
} from "./chunk-MJVCWBRK.js";
import "./chunk-E55OAK2U.js";
import {
  require_acorn_jsx
} from "./chunk-3QX6WZJS.js";
import {
  ancestor,
  base,
  simple
} from "./chunk-LTCZW2YY.js";
import {
  Parser,
  init_acorn
} from "./chunk-WL5MHHNK.js";
import {
  $a,
  Dg,
  La,
...
import {
  require_prop_types
} from "./chunk-HSWNNVPX.js";
import "./chunk-OG4FL6NF.js";
import {
  require_react
  ...
  try {
    switch (type.name) {
      ...
      case "arrayOf": {
        ...
        return compact != null && !Qe(compact) ? y(compact) : full ? y(short, full) : y(short);
      }
      case "func": {
        let { short, full } = generateType(type, extractedProp), summary = short, detail;
        return full && full.length < MAX_FUNC_LENGTH ? summary = full : full && (detail = toMultilineSignature(full)), y(summary, detail);
      }
      default:
        return null;
    }
...
function generateObject({ inferredType, ast }) {
  ...
  if (depth === 1) {
    ...
    if (!$a(compactObject)) return y(compactObject);
  }
  return y(OBJECT_CAPTION, generateObjectCode(ast));
}
function generateArray2({ inferredType, ast }) {
  let { depth } = inferredType;
  if (depth <= 2) {
    let compactArray = generateArrayCode(ast, true);
    if (!$a(compactArray)) return y(compactArray);
  }
  return y(ARRAY_CAPTION, generateArrayCode(ast));
}
...
function generateFunc2({ inferredType, ast }) {
  let { identifier } = inferredType;
  if (identifier != null) return y(getPrettyIdentifier(inferredType), generateCode(ast));
  let prettyCaption = generateCode(ast, true);
  return $a(prettyCaption) ? y(FUNCTION_CAPTION, generateCode(ast)) : y(prettyCaption);
}
function generateElement(defaultValue, inspectionResult) {
  ...
  if (identifier != null && !isHtmlTag(identifier)) {
    ...
    return y(prettyIdentifier, defaultValue);
  }
  return $a(defaultValue) ? y(ELEMENT_CAPTION, defaultValue) : y(defaultValue);
}
...
var stringResolver = (rawDefaultProp) => y(JSON.stringify(rawDefaultProp));
function generateReactObject(rawDefaultProp) {
  ...
  if (displayName != null) {
    ...
    return y(prettyIdentifier, jsx2);
  }
  if ((0, import_isString.default)(type) && isHtmlTag(type)) {
    ...
    if (!$a(jsxSummary)) return y(jsxSummary);
  }
  return y(ELEMENT_CAPTION, jsx2);
}
...
var functionResolver = (rawDefaultProp, propDef) => {
  ...
  else if (rawDefaultProp.prototype != null && (0, import_isFunction.default)(rawDefaultProp.prototype.render)) isElement = true;
  else {
    let innerElement;
    try {
  ...
  if (funcName != null) {
    if (isElement) return y(getPrettyElementIdentifier(funcName));
    inspectionResult != null && (inspectionResult = inspectValue(rawDefaultProp.toString()));
    let { hasParams } = inspectionResult.inferredType;
    return y(getPrettyFuncIdentifier(funcName, hasParams));
  }
  return y(isElement ? ELEMENT_CAPTION : FUNCTION_CAPTION);
};
var defaultResolver = (rawDefaultProp) => y(rawDefaultProp.toString());
...
function keepOriginalDefinitionOrder(extractedProps, component) {
  ...
  return propTypes != null ? Object.keys(propTypes).map((x) => extractedProps.find((y2) => y2.name === x)).filter(Boolean) : extractedProps;
}
...
function getPropDefs(component, section) {
  ...
  !Na(component) && !component.propTypes && isMemo2(component) && (processedComponent = component.type);
  let extractedProps = av(processedComponent, section);
...
var renderJsx = (code, options) => {
  ...
  typeof (options == null ? void 0 : options.displayName) == "string" ? displayNameDefaults = { showFunctions: true, displayName: () => options.displayName } : displayNameDefaults = { displayName: (el) => {
    var _a;
    return el.type.displayName ? el.type.displayName : La(el.type, "displayName") ? La(el.type, "displayName") : ((_a = el.type.render) == null ? void 0 : _a.displayName) ? el.type.render.displayName : typeof el.type == "symbol" || el.type.$$typeof && typeof el.type.$$typeof == "symbol" ? getReactSymbolName(el.type) : el.type.name && el.type.name !== "_default" ? el.type.name : typeof el.type == "function" ? "No Display Name" : isForwardRef2(el.type) ? el.type.render.name : isMemo2(el.type) ? el.type.type.name : el.type;
  } };
...
var skipJsxRender = (context) => {
  ...
  return (sourceParams == null ? void 0 : sourceParams.type) === Dg.DYNAMIC ? false : !isArgsStory || (sourceParams == null ? void 0 : sourceParams.code) || (sourceParams == null ? void 0 : sourceParams.type) === Dg.CODE;
};
var isMdx = (node) => {
  var _a, _b;
...
var jsxDecorator = (storyFn, context) => {
  ...
  (0, import_preview_api.useEffect)(() => {
    if (!skip) {
      ...
      channel.emit(yv, { id, source: jsx2, args: unmappedArgs });
    }
  });
  let story = storyFn();
...
var parameters = { docs: { story: { inline: true }, extractArgTypes, extractComponentDescription: iv } };
var decorators = [jsxDecorator];
var argTypesEnhancers = [cv];
export {
  applyDecorators,
  argTypesEnhancers,
  decorators,
